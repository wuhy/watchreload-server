...
function WatchServer(options) {
    ...
    // 设置依赖管理信息
    this.resDepManage = require('./resource-dependence');
    this.resDepManage._server = this;
}
...
WatchServer.prototype.initFileWatcher = function () {
...
WatchServer.prototype.initWebServer = function () {
    var config = this.config;
 ...
 */
WatchServer.prototype.start = function (httpServer) {
    var config = this.config;
    ...
    this.webServer.start(httpServer);
};
    ...
    this.webServer.pushCommandMessage(
        {
            ...
            data: {
                ...
                hmr: !!config.hmr,
                forceReloadWhenCssNotMatch: config.forceReloadWhenCssNotMatch
            }
        },
        client
    );
