...
function getReloadFile(changeFilePath) {
...
function preprocessFilePath(server, filePath, fileInfo) {
    var filePathProcessor = server.config.filePathProcessor;
    if (typeof filePathProcessor === 'function') {
        return filePathProcessor(filePath);
    }

    if (fileInfo.isScript && !/\.js$/.test(filePath)) {
        return filePath + '.js';
    }
    return filePath;
}
...
function sendFileRemoveMsg(server, filePath, fileInfo) {
    server.sendCommandMessage({
        ...
        path: preprocessFilePath(server, filePath, fileInfo)
    });
}
...
function sendFileChangeMsg(server, filePath, fileInfo) {
    var processedFilePath = preprocessFilePath(server, filePath, fileInfo);
    fileInfo.path = processedFilePath;
    server.sendCommandMessage({
...
function processFileChange(server, filePath, fileInfo, defaultProcessor) {
    var reloadFile = getReloadFile(filePath);
    if (reloadFile) {
...
exports.csWatchHandler = {
    ...
    connection: function (socket, count) {
        log.info(
            chalk.magenta('Client connected: ')
            + chalk.yellow(socket.id)
            + chalk.gray(', total connection count: ' + count)
        );
    ...
    disconnect: function (socket) {
        log.info(chalk.gray('Client %s disconnected...'), chalk.yellow(socket.id));
    },
    ...
    register: function (socket, data) {
        log.info(chalk.magenta('Connected client info: ')
            + chalk.gray(data.name));
    },
...
exports.fileWatchHandler = {
     ...
     */
    watchReady: function () {
        log.info('Watching files change start...');
    },
