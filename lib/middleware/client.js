 ...
 * @const
 */
var DEFAULT_CLIENT_SCRIPT = pathUtil.resolve(
    require.resolve('watchreload.js'), '../dist/watchreload'
);
 ...
 * @const
 */
var SOCKET_IO_CLIENT_SCRIPT = pathUtil.resolve(
    require.resolve('socket.io'), '../node_modules/socket.io-client/socket.io'
);
...
function rewriteBrowserReloadScript(script, options) {
    var socketUrl = 'http://' + options.ip + ':' + options.port;
    return new Buffer('\n' + script.replace(
        /\{\{socketUrl\}\}/g, socketUrl
    ));
}
 ...
 */
function readClientScripts(options, callback) {
    var scriptPaths = getClientScriptPaths(options);
...
function handleClientScriptRequest(request, response, options) {
    ...
    readClientScripts(options, function (err, result) {
        if (err) {
        ...
        var compressResult = compressResponseData(request, data);
